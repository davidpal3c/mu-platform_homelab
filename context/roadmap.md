...
Install and stabilize k3s; ensure container runtime and kubelet data use platform-tier bind mounts (TASK-0003). Baseline Kubernetes namespaces for this task are locked to: **`ceiba`**, **`aluna-context`**, **`aluna-mira`**, **`aluna-terra`**, **`observability`** — do not introduce new **`opencontext-*`** namespaces during bootstrap.
